package Ejercicios.src.ordenacion;
...
public class ArregloSeleccion {
    ...
    // ---------------------------------------------------------------------------------------------------
    // borra elemento del arreglo que sea igual al valor del argumento dato
    // desplazando los valores del
    ...
    // ----------------------------------------------------------------------------------------------------
    // metodo que ordena un arreglo ascendentemente usando la ordenacion por
    // seleccion
    public void ordenacionSeleccion() {
        ...
        for (int i = 0; i < numElems - 1; i++) { // el valor de i establece el comienzo de cada recorrido (bucle
                                                 // externo)
            minimo = i; // al inicio de cada recorrido el valor de minimo se inicializa apuntando al
                        // primer elemento
            for (int j = i + 1; j < numElems; j++) { // recorremos cada uno de los elementos del arreglo a partir del
                                                     // indice i+1 (bucle interno)
                if (arr[j] < arr[minimo]) { // comparamos elemento en el indice j con el actual valor minimo
                    minimo = j; // el elemento en la posicion j es menor que el actual valor minimo; elemento en
                                // la posicion j
                                // es el nuevo minimo
                }
            }
    ...
    public void eliminarElementosRepetidos() {
        ...
        for (int i = 0; i < numElems; i++) { // recorremos todos los elementos del arreglo arr
            for (j = 0; j < numElemsTemp; j++) // recorremos arreglo temporal buscando el elemento en la posicion i del
                                               // arreglo arr
                if (arr[i] == temp[j]) // si el elemento en la posicion i del arreglo arr se encuentra en el arreglo
                                       // temporal temp
                    break; // no seguimos recorriendo arreglo temporal porque se encontro elemento

            if (j == numElemsTemp) { // si el indice j llega hasta el final del arreglo temporal el elemento no esta
                                     // repetido
                temp[numElemsTemp] = arr[i]; // guardamos elemento del arreglo arr en la posicion i en arreglo temporal
                                             // (en la posicion k)
    ...
    // -----------------------------------------------------------------------------------------------------
    // metodo que muestra los elementos del arreglo que tengan una diferencia de dos
    // unidades en pares
    public void mostrarParesOrdenados() {
        ...
        while (i < numElems) { // mientras no lleguemos al final del arreglo arr
            diferencia = arr[i] - arr[j]; // calculamos diferencia entre elemento en la posicion i de arr y el
                                          // minimoValor
